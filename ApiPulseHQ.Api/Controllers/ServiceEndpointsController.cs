using ApiPulseHQ.Api.Models.ServiceEndpoints;
using ApiPulseHQ.Api.Services.ServiceEndpoints;
using Microsoft.AspNetCore.Authorization;
using Microsoft.AspNetCore.Mvc;

namespace ApiPulseHQ.Api.Controllers
{
    [ApiController]
    [Route("service-endpoints")]
    [Authorize] // 🔒 Protected
    public class ServiceEndpointsController : ControllerBase
    {
        private readonly IServiceEndpointsService _service;

        public ServiceEndpointsController(IServiceEndpointsService service)
        {
            _service = service;
        }

        private Guid GetUserId()
        {
            var userId = User.FindFirst("userId")?.Value;
            return Guid.Parse(userId!);
        }

        // GET: /service-endpoints
        [HttpGet]
        public async Task<IActionResult> GetAll()
        {
            var userId = GetUserId();
            var result = await _service.GetAllAsync(userId);
            return Ok(result);
        }

        // GET: /service-endpoints/{id}
        [HttpGet("{id:guid}")]
        public async Task<IActionResult> GetById(Guid id)
        {
            var userId = GetUserId();
            var result = await _service.GetByIdAsync(userId, id);

            if (result == null)
                return NotFound();

            return Ok(result);
        }

        // POST: /service-endpoints
        [HttpPost]
        public async Task<IActionResult> Create(CreateServiceEndpointRequest request)
        {
            var userId = GetUserId();
            var result = await _service.CreateAsync(userId, request);
            return Ok(result);
        }

        // PUT: /service-endpoints/{id}
        [HttpPut("{id:guid}")]
        public async Task<IActionResult> Update(Guid id, UpdateServiceEndpointRequest request)
        {
            var userId = GetUserId();
            var result = await _service.UpdateAsync(userId, id, request);

            if (result == null)
                return NotFound();

            return Ok(result);
        }

        // DELETE: /service-endpoints/{id}
        [HttpDelete("{id:guid}")]
        public async Task<IActionResult> Delete(Guid id)
        {
            var userId = GetUserId();
            var success = await _service.DeleteAsync(userId, id);

            if (!success)
                return NotFound();

            return Ok(new { message = "Deleted successfully" });
        }
    }
}
